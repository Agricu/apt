...
$('head').append('<link rel="stylesheet" href="Other/'+theme+'.css?_=f" type="text/css" />');
...
function loadPackageInfo() {
	...
	if (navigator.userAgent.search(/Cydia/) == -1) {
		$("#showAddRepo_").show();
		$("#open_cydia").attr("href","cydia://url/https://cydia.saurik.com/api/share#?source=http://repo.agricu.cn/&package="+urlSelfParts[1]);
	}
	$.ajax({
		url: form_url,
		type: "GET",
		...
		success: function (returnhtml) {
			...
			if(decodeResp.compatitle) {
				...
				var ios_ver = iOSVersion();
					$("#compatitle").html("iOS"+ios_ver);
				
			}



         if(decodeResp.otherurl) {
				$("#otherurl").html(decodeResp.otherurl);
				$("#otherurl_").show();
			}
          ...
          if(decodeResp.Device) {
				$("#Device").html(decodeResp.Device);
				$("Device_").show();
			}
...
{
	var sectionContent = "";
	sectionContent += "<li class=\"has-icon\"><a onclick=\"loadMainSection()\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"Other\back.png\" alt=\"\" srcset=\"Other/back.png 2x, Other/back.png 3x\" class=\"icon\"/><label>返回上一页</label></a></li>";
	for (var keyNow in packagesSection[sectionName]) {
		...
		sectionContent +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"Other/"+encodeURI(sectionName)+".png\" alt=\"\" srcset=\"Other/"+encodeURI(sectionName)+".png 2x, Other/"+encodeURI(sectionName)+".png 3x\" class=\"icon\"/><label>"+dicNow.package+" v"+dicNow.version+"</label></a></li>";
	}
...
function loadMainSection()
{
	...
	for (var section in packagesSection) {		
		sectionContent += "<li class=\"has-icon\"><a onclick=\"openSection('"+section+"')\" role=\"button\"><img style=\"border-radius: 20%;\" href=\"Other/"+encodeURI(section)+".png\" alt=\"\" srcset=\"Other/"+encodeURI(section)+".png 2x, Other/"+encodeURI(section)+".png 3x\" class=\"icon\"/><label>"+section+" ("+packagesSection[section].length+")</label></a></li>";
	}
	$("#browser").html(sectionContent);
}
...
function loadPackageDetail()
{
	...
	if(packageInfo!=null) {
		...
		$("#pkg_depends").html(packageInfo.depends); 
      $("#pkg_author").html(packageInfo.author);
      $
("#pkg_description").html(packageInfo.description);
...
function loadRecentUpdates()
{
	...
	for (var dicNow in allPackages) {
		...
		htmlnews +=  "<li class=\"has-icon\"><a href='"+urlOpen+"' target='_blank' role=\"button\"><img style=\"border-radius: 20%;\" href=\"Other/"+encodeURI(allPackages[dicNow].section)+".png\" alt=\"\" srcset=\"Other/"+encodeURI(allPackages[dicNow].section)+".png 2x, Other/"+encodeURI(allPackages[dicNow].section)+".png 3x\" class=\"icon\"/><label>"+allPackages[dicNow].package+" v"+allPackages[dicNow].version+"</label></a></li>";
	}
